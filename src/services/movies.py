...
_ENTITY_LABELS = {"genres": "Genre", "stars": "Star", "directors": "Director", "certifications": "Certification"}
_NOT_FOUND_BY_TABLE = {
...
class MovieService:
    ...
    async def _paginate(self, stmt, page: int, per_page: int) -> PaginatedResponseSchema[MovieListItemSchema]:
        db = self.repo.db
        count_stmt = stmt.order_by(None).with_only_columns(func.count()).select_from(stmt.subquery())
    ...
    async def filter_movies(self, filters: MovieFilterParams, page: int, per_page: int):
        stmt = self.repo.apply_filters(self.repo.base_query(), filters).order_by(Movie.id)
        return await self._paginate(stmt, page, per_page)

    async def sort_movies(self, sort_by: MovieSortField, order: SortOrder, page: int, per_page: int):
        stmt = self.repo.apply_sort(self.repo.base_query(), sort_by, order)
        return await self._paginate(stmt, page, per_page)

    async def search_movies(self, query: str, page: int, per_page: int):
        stmt = self.repo.apply_search(self.repo.base_query(), query).order_by(Movie.id)
        return await self._paginate(stmt, page, per_page)
    ...
    async def create_movie(self, payload: MovieCreateSchema) -> MovieDetailSchema:
        movie = Movie(**payload.model_dump(exclude={"genre_ids", "star_ids", "director_ids"}))
        ...
        movie.directors = await self.repo.get_directors_by_ids(payload.director_ids)
        if not await self.repo.create_movie(movie):
            raise MovieAlreadyExistsError(
                f"Movie «{payload.name}» ({payload.year}, {payload.time} min) already exists."
            )
        return await self.get_detail(movie.id)

    async def update_movie(self, movie_id: int, payload: MovieUpdateSchema) -> MovieDetailSchema:
        movie = await self.repo.get_by_id(movie_id)
        if movie is None:
            raise MovieNotFoundError(f"Movie id={movie_id} didn't find.")
        data = payload.model_dump(exclude_unset=True, exclude={"genre_ids", "star_ids", "director_ids"})
        ...
        if not await self.repo.save_movie(movie):
            raise MovieAlreadyExistsError(f"Movie «{movie.name}» ({movie.year}, {movie.time} min) already exists.")
    ...
    async def delete_movie(self, movie_id: int) -> None:
        ...
        if await self.repo.has_purchases(movie_id):
            raise MovieHasPurchasesError(f"Movie id={movie_id} already sold — deletion is prohibited.")
    ...
    async def list_genres_with_count(self) -> list[GenreWithCountSchema]:
        ...
        return [GenreWithCountSchema(id=g.id, name=g.name, movies_count=c) for g, c in rows]

    async def get_movies_by_genre(self, genre_id: int, page: int, per_page: int):
        if await self.repo.get_genre_by_id(genre_id) is None:
            raise GenreNotFoundError(f"Genre id={genre_id} didn't find.")
        return await self._paginate(self.repo.movies_by_genre_query(genre_id), page, per_page)
    ...
    async def remove_from_favorites(self, movie_id: int, user_id: int) -> None:
        ...
        if favorite is None:
            raise FavoriteNotFoundError(f"Movie id={movie_id} not in favorites.")
    ...
    async def filter_favorites(self, user_id: int, filters: MovieFilterParams, page: int, per_page: int):
        stmt = self.repo.apply_filters(self.repo.favorites_base_query(user_id), filters).order_by(Movie.id)
        return await self._paginate(stmt, page, per_page)

    async def sort_favorites(self, user_id: int, sort_by: MovieSortField, order: SortOrder, page: int, per_page: int):
        stmt = self.repo.apply_sort(self.repo.favorites_base_query(user_id), sort_by, order)
        return await self._paginate(stmt, page, per_page)

    async def search_favorites(self, user_id: int, query: str, page: int, per_page: int):
        stmt = self.repo.apply_search(self.repo.favorites_base_query(user_id), query).order_by(Movie.id)
        return await self._paginate(stmt, page, per_page)

    async def like_movie(self, movie_id: int, user_id: int, status_: LikeStatus) -> None:
        if await self.repo.get_by_id(movie_id) is None:
            raise MovieNotFoundError(f"Movie id={movie_id} didn't find.")
        await self.repo.upsert_movie_like(movie_id, user_id, status_)

    async def rate_movie(self, movie_id: int, user_id: int, rating: int) -> None:
        if await self.repo.get_by_id(movie_id) is None:
            raise MovieNotFoundError(f"Movie id={movie_id} didn't find.")
    ...
    async def create_comment(self, movie_id: int, user_id: int, payload: CommentCreateSchema) -> CommentSchema:
        parent = None
        if payload.parent_id is not None:
            parent = await self.repo.get_comment(payload.parent_id)
            if parent is None:
                raise CommentNotFoundError(f"Parental comment id={payload.parent_id} didn't find.")
        comment = Comment(movie_id=movie_id, user_id=user_id, parent_id=payload.parent_id, text=payload.text)
    ...
    async def create_dictionary_item(self, model, name: str):
        ...
        if not ok:
            ...
            raise DictionaryItemAlreadyExistsError(f"{entity} with name «{name}» already exists.")
        return item
