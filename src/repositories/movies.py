...
from schemas.movies import MovieFilterParams, MovieSortField, SortOrder
...
class MovieRepository:
    ...
    def apply_filters(self, stmt, filters: MovieFilterParams):
    ...
    def apply_sort(
            self, stmt, sort_by: MovieSortField, order: SortOrder
    ):
    ...
    def apply_search(self, stmt, query: str):
        pattern = f"%{query}%"
        return (
    ...
    async def count_likes(
            self, movie_id: int, status_: LikeStatus
    ) -> int:
        stmt = select(func.count()).where(
            MovieLike.movie_id == movie_id,
            MovieLike.status == status_
        )
        result = await self.session.execute(stmt)
    ...
    async def get_directors_by_ids(
            self, ids: list[int]
    ) -> list[Director]:
    ...
    async def list_genres_with_movie_count(self):
        stmt = (
            select(Genre, func.count(MovieGenre.movie_id)
                   .label("movies_count"))
    ...
    def movies_by_genre_query(self, genre_id: int):
        return select(Movie).join(Movie.genres).where(
            Genre.id == genre_id).order_by(Movie.id
                                           )
    ...
    def favorites_base_query(self, user_id: int):
        return (select(Movie).join(
